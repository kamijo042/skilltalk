          ...
          <div class="osahan-privacy bg-white rounded shadow-sm p-4">
            <div id="yourContent" class="active">
              ...
              <table>
                ...
                <tr>
                  <td><a href="{{route('mypage.lecture-own.message.thread', ['message_thread_id' => $message_thread->id])}}">{{$message_thread->title}}</a></td>
                  <td>{{$message_thread->published_name}}</td>
                  <td>{{$thread_status[$message_thread->entry_status]}}</td>
                  <td>{{date('Y/m/d', strtotime($message_thread->last_updated))}}</td>
                </tr>
                @endforeach
              </table>
